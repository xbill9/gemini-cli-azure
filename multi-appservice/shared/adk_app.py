...
def main(
    ...
    try:
        if a2a:
            _prepare_a2a_agent_cards(agents_dir, files_to_cleanup, folders_to_cleanup, effective_agents, port)
        ...
        @app.get("/")
        async def root():
            return {"status": "ok", "service": os.path.basename(agents_dir)}
...
def _prepare_a2a_agent_cards(
    agents_dir: str, files_to_cleanup: list, folders_to_cleanup: list, effective_agents: list[str] | None = None, port: int = 8000
):
    """Generates temporary agent.json cards for A2A discovery."""
    print(f"DEBUG: _prepare_a2a_agent_cards starting with port={port}")
    ...
    for agent_name in agents:
        ...
        # Always rebuild the card to ensure correct URL/port
        try:
        ...
        # Use 127.0.0.1 and the explicit port for local communication
        card_builder = AgentCardBuilder(
            agent=agent,
            rpc_url=f"http://127.0.0.1:{port}/a2a/{agent_name}",
