...
*   **Model:** `google/gemma-4-26B-A4B-it` (MoE Optimized)
*   **Azure Orchestration:** Active (using `gpt-5-nano`)
...
### 2. Azure OpenAI (Cost-Effective Orchestration)
The agent is optimized to use the **cheapest Azure LLM model** for diagnostic and orchestration tasks:
*   **Model:** `gpt-5-nano` (or `gpt-4o-mini`)
*   **Pricing:** ~$0.05 per 1M tokens.
*   **Setup:** Ensure `AZURE_API_KEY` and `AZURE_API_BASE` are set in your `.env`.
