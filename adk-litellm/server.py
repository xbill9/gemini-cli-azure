...
@mcp.tool()
async def get_vllm_deployment_config(service_name: str = "vllm-gemma4", model_name: str = "google/gemma-4-26B-A4B-it") -> str:
    ...
    return (
        ...
        f"    sudo docker run -d --name vllm-gemma4 --privileged --net=host -v /dev/shm:/dev/shm \\\n"
        f"    -e HF_TOKEN=$(gcloud secrets versions access latest --secret=hf-token) \\\n"
        f"    vllm/vllm-tpu:nightly vllm serve {model_name} --tensor-parallel-size 4'"
    )


@mcp.tool()
async def get_azure_deployment_config() -> str:
    """Generates instructions for deploying the cheapest Azure LLM model (GPT-5-nano)."""
    return (
        "### 🚀 Azure OpenAI Deployment (GPT-5-nano)\n"
        "1. **Resource:** Create an Azure OpenAI resource in a supported region (e.g., East US).\n"
        "2. **Model Catalog:** Navigate to Azure AI Foundry Model Catalog.\n"
        "3. **Select Model:** Search for `gpt-5-nano`.\n"
        "4. **Deploy:** Select 'Serverless API' (Pay-as-you-go).\n"
        "5. **Endpoint:** Note your Target URL and API Key.\n\n"
...
async def analyze_cloud_logging(minutes: int = 60) -> str:
    """Summarizes TPU-related errors using the cheapest Azure LLM model."""
    log_filter = f'resource.type="tpu_worker" AND timestamp >= "{(time.time() - minutes*60)}"'
    logs = await get_cloud_logging_logs(log_filter=log_filter, limit=50)
    
    prompt = f"Analyze the following Cloud Logs for errors and suggest remediations:\n\n{logs}"
    
    # Try Azure first for cheapest analysis
    azure_key = os.getenv("AZURE_API_KEY")
    azure_base = os.getenv("AZURE_API_BASE")
    if azure_key and azure_key != "your-azure-key" and azure_base:
        try:
            ...
            chat_completion = await client.chat.completions.create(
                ...
                model=os.getenv("AZURE_MODEL", "gpt-5-nano"),
            )
            return f"### 🤖 Azure AI Analysis (gpt-5-nano)\n{chat_completion.choices[0].message.content}"
        except Exception as e:
            logger.error(f"Azure analysis failed: {e}. Falling back to TPU.")
            return f"⚠️ Azure Analysis failed: {e}\n\n" + await query_queued_gemma4(prompt)
    
    return await query_queued_gemma4(prompt)
...
@mcp.tool()
async def orchestrate_gemma4_stack(resource_id: str = "vllm-gemma4-qr") -> str:
    """Seamlessly provisions a TPU Queued Resource and deploys the optimized vLLM stack."""
    # This is a high-level orchestrator
    status = await get_system_status()
    if "🟢 Online" in status:
        return "✅ Gemma 4 stack is already ONLINE."
    
    return await manage_queued_resource(resource_id)
