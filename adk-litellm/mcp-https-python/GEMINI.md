...
## Project Overview
This is a **Python-based Model Context Protocol (MCP) server** using the `FastMCP` class from the `mcp` SDK. It is designed to expose tools (like `manage_queued_resource`) over HTTP (using streamable-http) for integration with MCP clients (such as Claude Desktop or Gemini clients).
...
do not recommend 2.0 flash or older models they are depreciated

## Project Structure

*   `main.py`: The entry point of the application. Initializes the `FastMCP` server ("Queued TPU vLLM Agent (Gemma 4)") and defines tools.
...
The server is configured to run using the `streamable-http` transport on `http://0.0.0.0:8080` (port configurable via `PORT` env var).
