...
-   **Video Frame Rate**: Controlled via `VIDEO_FPS` environment variable (default: 2.0). Range: 0.5 to 5.0 Hz.
-   **Instruction Sync**: The agent's instructions are dynamically generated to match the configured `VIDEO_FPS`, ensuring the model's analysis frequency aligns with the actual stream.
...
5.  **Deployment:** Ensure all environment variables (especially `MODEL_ID` and `GOOGLE_API_KEY`) are correctly set in the Azure Kubernetes Service (AKS) configuration.
    -   **Manual Deployment**: Use `make deploy-aks` to deploy directly to Azure Kubernetes Service (AKS).
    - **Resource Group**: Default is `gemini31`.
    -   **Location**: Default is `eastus`.
    -   **Clean Up**: Use `make destroy-aks` to remove the AKS resources or `make az-destroy` to remove the entire resource group.
