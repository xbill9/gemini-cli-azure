# MCP HTTP Python Server for Azure Container Apps (ACA)

A simple Model Context Protocol (MCP) server implemented in Python using `FastMCP`, optimized for deployment to **Azure Container Apps (ACA)**. This server communicates over `HTTP` and serves as a foundational "Hello World" example for Python-based MCP integrations.
...
The project includes a `Makefile` to automate deployment to Azure Container Apps (ACA) using containers.
...
2.  **Deploy to Azure Container Apps (ACA):**
    ...
    *This command performs the following:*
    ...
    - Deploys the container to Azure Container Apps (ACA).
