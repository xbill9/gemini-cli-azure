...
We provide an extensive, automated [Makefile](file:///home/xbill/gemini-cli-aws/midsommer-wasm-rust-aws/Makefile) to run, compile, test, and deploy the entire hybrid stack:
...
| `make clean` | Cleans Flutter build directories, Rust target build cache, and intermediate outputs. |
| `make docker-build` | Builds the local web server Docker container packaging the static game assets. |
| `make deploy` | Compiles WASM, builds the Docker image, pushes it to Azure Container Registry (ACR), and initiates a new Azure Container App (ACA) deployment. |
| `make aca-status` | Checks active service and deployment state on Azure Container App. |
| `make endpoint` | Fetches the active public FQDN URL from Azure Container App. |
| `make destroy` | Tears down the Container App, Environment, ACR, and Resource Group. |

---

## 🌐 Azure Container Apps Deployment

The web-based game client is served via a lightweight Rust/Axum static web server deployed on **Azure Container Apps (ACA)**:
* **Docker Containerization**: The [Dockerfile](file:///home/xbill/gemini-cli-azure/midsommer-wasm-rust-azure/Dockerfile) configures a multi-stage Docker build that compiles our Axum web server and bundles the game assets.
* **Public Endpoint**: The service runs on port `8080` with health checks bound to the `/health` endpoint, making the game accessible globally under the Azure Container App's public FQDN domain.
