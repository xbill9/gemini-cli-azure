...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::registry()
        .with(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,mcp_aca_rust_azure=debug,rmcp=debug".into()),
        )
        .with(
    ...
    if let Ok(hosts_str) = std::env::var("ALLOWED_HOSTS") {
        ...
        // Default allowed hosts - in ACA we often need * or the FQDN
        tracing::warn!("ALLOWED_HOSTS env not set, defaulting to * for cloud compatibility");
        config.allowed_hosts = vec![];
    }

    let service = StreamableHttpService::new(service_factory, session_manager.into(), config);

    // Add a specific health check route, nest the MCP service, and serve static game assets from /assets
    let app = axum::Router::new()
        .route("/health", axum::routing::get(|| async { "ok" }))
        .nest_service("/mcp", service)
        .fallback_service(tower_http::services::ServeDir::new("assets"));

    // Determine port from environment variable (Cloud Run/ACA standard)
